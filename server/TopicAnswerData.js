...
var selectByID = function (id) {
    ...
    connection.connect();
    connection.query('select  *  from  `topic_answer_view` where id=?', [id], function (err, rows, fields) {
        if (err) {
            defer.reject(err);
        }
        else {
...
var selectByAll = function () {
    ...
    var connection = mysql.createConnection(mysqlOP.op);
    connection.connect();
    connection.query('select  *  from   `topic_answer_view` ', function (err, rows, fields) {
        if (err) {
            defer.reject(err);
        }
        else {
